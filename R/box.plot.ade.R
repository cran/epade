...
alt.t<-NULL
for(k in 1:n.g2){
if(!is.null(group2)) x.k<- x[g2==levels(g2)[k]]
if(!is.null(group2)) g.k<- g[g2==levels(g2)[k]]
...
if(all(skewness.k<=1)) {
p.k<- format_p.ade(t.test(x.k[g.k==levels(g.k)[1]],x.k[g.k==levels(g.k)[2]])$p.value , 4)
pstri<-'T-Test: '
}
if(any(skewness.k>1)){
p.k<- format_p.ade(wilcox.test(x.k[g.k==levels(g.k)[1]],x.k[g.k==levels(g.k)[2]])$p.value , 4)
pstri<-'U-Test: '
}
p<-c(p, p.k)
}
...
count.n<-NULL
for(k in 1:n.g2){
if(!is.null(group2)) bstats[[k]]<-boxplot(x[g2==levels(g2)[k]] ~ g[g2==levels(g2)[k]], plot=FALSE, pch=16, cex=1.25, col=rgb(1,1,1), staplewex=c(0.25,0.25,0.25,0.25), boxwex=c(0.75,0.75,0.75,0.75), ylim=ylim, xlim=xlim, names=NA, varwidth=varwidth)
...
if(!is.null(yticks) & length(yticks)==1)   axis(2, at=pretty(x, n=yticks),          labels =TRUE,       tick=TRUE, col=a.coladd.ade(bgcol, -75), lwd.ticks=1, lwd=0)
if(!is.null(yticks) & length(yticks)>1 )   axis(2, at=yticks,                       labels =TRUE,       tick=TRUE, col=a.coladd.ade(bgcol, -75), lwd.ticks=1, lwd=0)
if(is.null(yticks) )                       axis(2,                                  labels =TRUE,       tick=TRUE, col=a.coladd.ade(bgcol, -75), lwd.ticks=1, lwd=0)
...
if(!is.null(yticks) & length(yticks)==1)   axis(2, at=pretty(x, n=yticks),          labels =TRUE,       tick=TRUE, col=a.coladd.ade(bgcol, -50), lwd.ticks=1, lwd=0)
if(!is.null(yticks) & length(yticks)>1 )   axis(2, at=yticks,                       labels =TRUE,       tick=TRUE, col=a.coladd.ade(bgcol, -50), lwd.ticks=1, lwd=0)
if(is.null(yticks) )                       axis(2,                                  labels =TRUE,       tick=TRUE, col=a.coladd.ade(bgcol, -50), lwd.ticks=1, lwd=0)
